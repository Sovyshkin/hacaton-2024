  ...
  methods: {
    async goSearch() {
      ...
      if (response.data.search) {
        let entry = JSON.parse(response.data.search.replace(/'/g, '"'));
        this.$router.push({ name: "entry", query: { id: entry.id } });
      }
    },

    async load_info() {
      let response = await axios.post(`/lenta`);
...
<template>
  ...
  <div class="group-search">
    <input v-model="dataSearch" type="text" placeholder="поиск..." />
    <img @click="goSearch" src="../assets/search.png" alt="" />
  </div>
  <div class="news">
